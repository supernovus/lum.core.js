...
const ApplyWith =
{
  // A private init function for cpHandler.
  cpInit(opts)
  {
    opts.cp = copyProps.cache.into(opts.target);
    opts.cp.applyDone = true;
  },
  ...
  cpHandler(info)
  {
    ...
    { // Register the handler.
      ApplyWith.cpInit(info);
      info.handlers.add(ApplyWith.cpHandler);
    }
    else if (info[ApplyInfo])
    { // Handle objects
      let {opts, value} = info;
      if (isObj(value))
      {
        if (!opts.cp) ApplyWith.cpInit(opts);
        opts.cp.from(value);
        info.ok = true;
        info.done = opts.cp.applyDone;
      }
    }
  },
