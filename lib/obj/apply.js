...
const ApplyWith =
{
  ...
  v1Args()
  {
    let opts = ApplyWith.getOpts(arguments);
    if (opts)
    {
...
function apply(target, ...values)
{
  const opts = 
  {
    ...
    ctx: target, 
    handlers: new Set(),
    // TODO: remove the obj getter/setter in v2.x
    get obj()
    {
      return deprecated('apply().obj', 'apply().target', this.target);
    },
    set obj(val)
    {
      this.target = val;
      return this.obj; // Trigger the deprecation message.
    },
  };
