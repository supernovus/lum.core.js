 ...
 * 
 * If this is a `function` it will be used as the `spec.init` value.
 * 
 * @param {function} spec.init - The function that returns the real value
...
function wrapDepr(obj,prop,spec)
{
  if (typeof spec === F)
    spec = {init: spec};
  if (typeof spec.init !== F) 
    throw new TypeError("invalid init");

  return def(obj, prop, 
  {
    get: () =>
      deprecated(spec.dep??prop, spec.rep, spec.init())
  }, spec.opts);
}
